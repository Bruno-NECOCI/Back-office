...
class GestionContratModel extends Database{
...
public function getInfosEvent()
	{
		...
		WHERE etat='devis' ORDER BY evenement.id")->fetchAll();
	}
	...
	public function getIdAndTypeOrga()
	{
		return $this->db->query("SELECT 
			...
			WHERE etat='devis'")->fetchAll(PDO::FETCH_ASSOC);
	...
	public function updateEtatProgress($id)
	{
		$this->db->query("UPDATE evenement SET etat = 'todo' WHERE id = ?", [$id]);
